...
function ode_for_testing_f!(
    ...
    du_cache_vec .= 0.0 #this is also some weird behaviour I've noticed, SparseConnectivityTracer always returns undef, so we're forced to do this
    u_cache_vec .= 0.0

    #NOT ABSOLUTELY REQUIRED
    #Note that the specific syntax of ComponentVector is not required and would be better if it was not required. 
...
properties_axes = getaxes(properties)
