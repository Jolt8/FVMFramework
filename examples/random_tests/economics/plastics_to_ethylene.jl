...
#can't find any good numbers on this, I'll just use FT which is probably pretty pessimistic 
function cost_per_kg_of_ethylene_based_on_kgs_of_capacity_of_plant(ethylene_capacity)
    cost_b = 20e9u"USD"
    capacity_b = 257.6u"kg/s"

    cost_of_desired_capacity_plant = cost_b * (ethylene_capacity / capacity_b)^0.65

    plant_lifetime_years = 25 
    plant_lifetime = plant_lifetime_years * 365 * 24 * 60 * 60u"s"

    total_kg_ethylene_produced = plant_lifetime * ethylene_capacity

    return cost_of_desired_capacity_plant / total_kg_ethylene_produced
end
...
function cost_per_kg_of_plastic_processed_based_on_kgs_of_capacity_of_plant(plastic_capacity)
    ...
    total_kg_plastic_processed = plant_lifetime * plastic_capacity

    plant_cost_per_kg_plastic_processed = cost_of_desired_capacity_plant / total_kg_plastic_processed
    ...
    return plant_cost_per_kg_plastic_processed + cost_of_energy_per_kg_plastic
end
