function generate_ray_map_between_nodes(transducer_ids, transducer_node_ids, grid, geo)
    ...
    for origin_transducer_id in transducer_ids
        ...
        for destination_transducer_id in transducer_ids
            ...
            if origin_transducer_id != destination_transducer_id #I'm lazy, so we're not slicing the array
                intersected_cells, corresponding_intersection_lengths, ray_length = find_traced_cells(origin_node_id, destination_node_id, grid, geo)
...
function generate_ray_map(transducer_ids, transducer_node_ids, grid, geo)
    ray_map_intersected_cells, ray_map_distances_through_cells, ray_map_ray_lengths = generate_ray_map_between_nodes(transducer_ids, transducer_node_ids, grid, geo)
