...
ADC_CLK       = 1    # GP1  — ADC clock output (Hardware PWM) [GP27 broken on RP2350]
ADC_OTR       = 28   # GP28 — ADC out-of-range indicator

# =============================================================================
# DEFAULT CAPTURE PARAMETERS
# =============================================================================
DEFAULT_N_SAMPLES    = 20000       # 200 µs capture window at 10 MSPS
...
def adc_capture_12bit():
    ...
    wait(1, gpio, 1)                # Wait for CLK HIGH (rising edge) — GP1
    wait(0, gpio, 1)                # Wait for CLK LOW  (falling edge) — GP1
...
# DMA CONFIGURATION
# =============================================================================
# DREQ for PIO0 SM1 RX = 5 on both RP2040 and RP2350 (TX0-3=0-3, RX0-3=4-7)
DREQ_PIO0_RX1 = 5
...
def capture_adc():
    ...
    sm1.active(0)
    sm1.restart() # Clear internal PIO shift registers
    while sm1.rx_fifo() > 0:
        sm1.get()
