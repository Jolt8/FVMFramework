...
module GasProperties
    ...
    function get_mixture_cp_mass(species_list, mole_fractions, T)
        cp_mix = 0.0u"J/(mol*K)"
        for (name, x) in zip(species_list, mole_fractions)
            if haskey(COEFFS, name)
                cp_mix += x * get_cp_molar(name, T)
...
struct PBRModel{T<:ThermalModel, P<:PressureModel}
    ...
    catalyst_linear_densities::Vector{typeof(1.0u"kg/m")}
    components::Vector{String}
    coolprop_components::Vector{String}
    heat_of_reactions_dict::Dict{String, typeof(1.0u"kJ/mol")}
end

function calculate_rates_and_properties(T, P, molar_flows_vec, p::PBRModel)
...
function calculate_dT_dL(p::PBRModel{MixedThermalModel}, T, P, common, dq_dL)
    reactions = p.reactions
    ...
    for i in eachindex(reactions)
        catalyst_linear_density = p.catalyst_linear_densities[i]
        println("reaction_name, ", reactions[i].name)
        println("reaction enthalpy, ", uconvert(u"W/m", heat_of_reactions_dict[reactions[i].name] * -net_reaction_rates[reactions[i].name] * catalyst_linear_density))
        println("reaction_rate, ", net_reaction_rates[reactions[i].name])
        println("reaction_rate -, ", -net_reaction_rates[reactions[i].name])
    ...
    fluid_molar_heat_capacity = GasProperties.get_mixture_cp(p.coolprop_components, common.z_vec, T)
    ...
    println("input_wattage_per_m, ", input_wattage_per_m)
...
function calculate_dP_dL(p::PBRModel{MixedThermalModel, DarcyWeisbachPressureDrop}, T, P, common)
    ...
    fluid_molar_density = GasProperties.get_mixture_molar_density(T, P)
    fluid_density = GasProperties.get_mixture_density(p.coolprop_components, common.z_vec, T, P)
    ...
    #println("temp, ", T)
    #println("pressure, ", P)
    #println("fluid_density, ", uconvert(u"kg/m^3", fluid_density))
    #println("fluid_velocity, ", uconvert(u"m/s", fluid_velocity))
    #println("channel_hydraulic_diameter, ", uconvert(u"m", channel_hydraulic_diameter))
    #println("dynamic_viscosity, ", dynamic_viscosity)

    Re = (fluid_density * fluid_velocity * channel_hydraulic_diameter) / (dynamic_viscosity)

    #println("Not Strip Re, ", (fluid_density * fluid_velocity * channel_hydraulic_diameter) / (dynamic_viscosity))
    #println("Re, ", Re)
...
function laminar_entry_Baehr_Stephan(Re, Pr, z, Di)
    ...
    println("Pr ", Pr)
    println("z ", z)
    println("pipe_hydraulic_diameter ", Di)
    eff_len = max(z, 1e-5u"m") 
    Gz = Di/eff_len*Re*Pr
...
function get_linear_heat_transfer_coefficient(
    ...
    if Re_2 <= 2200 
        ...
        #println("Pr_2 ", Pr_2)
        #println("z ", z)
        #println("pipe_2_hydraulic_diameter ", pipe_2_hydraulic_diameter)
...
function calculate_UA_prime_system(
    ...
    fluid_1_molar_density = GasProperties.get_mixture_molar_density(model_1_T, model_1_P)
    fluid_1_density = GasProperties.get_mixture_density(model_1.coolprop_components, model_1_common.z_vec, model_1_T, model_1_P)

    println("model_1.coolprop_components, ", model_1.coolprop_components)
    println("model_1_common.z_vec ", model_1_common.z_vec)
    println("model_1_T ", model_1_T)

    fluid_1_specific_heat = GasProperties.get_mixture_cp_mass(model_1.coolprop_components, model_1_common.z_vec, model_1_T)

    fluid_1_total_molar_flow = sum(model_1_molar_flows)
    
    fluid_1_total_volumetric_flow = fluid_1_total_molar_flow / fluid_1_molar_density

    fluid_1_velocity = fluid_1_total_volumetric_flow / model_1.pressure_model.channel_cross_sectional_area

    fluid_1_dynamic_viscosity = 3.5e-5u"Pa*s" #just using something close

    Re_1 = (fluid_1_density * fluid_1_velocity * model_1.pressure_model.channel_hydraulic_diameter) / (fluid_1_dynamic_viscosity)
    println("fluid_1_dynamic_viscosity ", fluid_1_dynamic_viscosity)
    println("fluid_1_specific_heat ", fluid_1_specific_heat)
    println("fluid_1_thermal_conductivity ", fluid_1_thermal_conductivity)
    Pr_1 = (fluid_1_dynamic_viscosity * fluid_1_specific_heat) / (fluid_1_thermal_conductivity)
...
function pbr_ode_system!(du, u, p::Vector, z)
    ...
    # Catalyst density assumption for rate conversion (mol/s/kg -> mol/s/m)
    catalyst_linear_density = 0.01u"kg/m"
    ...
    du[2] = 0.0u"bar/m"
    #calculate_dP_dL(smr_model, SMR_T, SMR_P, common_SMR)
    ...
    du[10] = 0.0u"bar/m"
    #calculate_dP_dL(comb_model, comb_T, comb_P, common_comb)
...
total_reactor_methane_mass_flow = 0.001u"g/s" 
SMR_to_combustor_methane_ratio = 3.0 #changed from 3 to 9
...
reactor_inlet_temperature = 450.13u"°C" |> u"K"
...
test = ["CH4", "H2O", "CO", "H2", "CO2", "Cs"]

fluid_molar_density = GasProperties.get_mixture_molar_density(T, P)
fluid_density = GasProperties.get_mixture_density(test, z_smr, T, P)
...
fluid_velocity = uconvert(u"m/s", fluid_velocity)
=#
# -- Physical Model Parameters --
input_wattage_per_m = 0u"W/m"
ambient_temperature = 25.13u"°C" |> u"K"
overall_heat_trans_coeff = 0.0u"W/(m^2*K)"
...
channel_cross_sectional_area = (pi * channel_radius^2) / 2
...
SMR_parameters = PBRModel(
    ...
    [0.01u"kg/m", 0.01u"kg/m", 0.01u"kg/m"], #catalyst_linear_densities
    ["CH4", "H2O", "CO", "H2", "CO2", "Cs"], # Component Order
    ["methane", "water", "CO", "hydrogen", "CO2", "carbon"], # Names
...
combustion_inlet_temperature = 450.0u"°C" |> u"K"
...
combustion_parameters = PBRModel(
    ...
    [Combustion_Reaction], 
    [0.0001u"kg/m"], #catalyst linear densities
    ["CH4", "O2", "N2", "CO2", "H2O"], # N2 is inert
    ["methane", "oxygen", "nitrogen", "CO2", "water"],
    Dict("Combustion" => -802.0u"kJ/mol"), 
)
#lowering the enthalpy of combustion let's the solver converge
...
jac_sparsity = ADTypes.jacobian_sparsity(
    (du, u) -> pbr_ode_system!(du, u, parameter_vec, 0.0), du0, u0, detector)
=#
#the above doesn't work because of exp(::Quantity{SparseConnectivityTracer.GradientTracer{Int64, BitSet}, 𝚯, Unitful.FreeUnits{(J^-1,  kJ, K), 𝚯, nothing}})

pbr_sol = DifferentialEquations.solve(pbr_ode_problem, Tsit5())#, saveat=(lspan[end]/desired_amount_of_u))

u_start = length(u0) - length(SMR_parameters.components) + 1

["methane", "water", "CO", "hydrogen", "CO2", "carbon"]


temperature_over_SMR_reactor = [u[1] for u in pbr_sol.u]
pressure_over_SMR_reactor = [u[2] for u in pbr_sol.u]

SMR_mole_flow_CH4_values = [u[3] for u in pbr_sol.u]
SMR_mole_flow_H2O_values = [u[4] for u in pbr_sol.u]
SMR_mole_flow_CO_values = [u[5] for u in pbr_sol.u]
SMR_mole_flow_H2_values = [u[6] for u in pbr_sol.u]
SMR_mole_flow_CO2_values = [u[7] for u in pbr_sol.u]
SMR_mole_flow_C_values = [u[8] for u in pbr_sol.u]

temperature_over_combustion_reactor = [u[9] for u in pbr_sol.u]
pressure_over_combustion_reactor = [u[10] for u in pbr_sol.u]

reactor_lengths = pbr_sol.t

plt = plot(reactor_lengths, SMR_mole_flow_CH4_values,
    xlabel="reactor length",
    ylabel="Molar Flow Through Reactor",
    title="PBR Species Molar Flow Through \n Reactor vs. reactor_lengths",
...
plot!(reactor_lengths, SMR_mole_flow_H2O_values, linewidth=2, marker=:circle, markersize=3, label="H2O")
plot!(reactor_lengths, SMR_mole_flow_CO_values, linewidth=2, marker=:circle, markersize=3, label="CO")
plot!(reactor_lengths, SMR_mole_flow_H2_values, linewidth=2, marker=:circle, markersize=3, label="H2")
plot!(reactor_lengths, SMR_mole_flow_CO2_values, linewidth=2, marker=:circle, markersize=3, label="CO2")
plot!(reactor_lengths, SMR_mole_flow_C_values, linewidth=2, marker=:circle, markersize=3, label="C")


plot(reactor_lengths, temperature_over_SMR_reactor, linewidth=2, marker=:circle, markersize=3, label="temperature")
plot!(reactor_lengths, temperature_over_combustion_reactor, linewidth=2, marker=:circle, markersize=3, label="temperature")
#plot(reactor_lengths, pressure_over_SMR_reactor, linewidth=2, marker=:circle, markersize=3, label="pressure")
#TODO
