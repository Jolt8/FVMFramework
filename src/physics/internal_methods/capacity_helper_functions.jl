...
function cap_heat_flux_to_temp_change!(
    ...
    # = K/s
    du.temp[cell_id] += du.heat[cell_id] / (cell_volumes[cell_id] * u.rho[cell_id] * u.cp[cell_id])
end
...
function cap_mass_flux_to_pressure_change!(
    ...
    #remember, J = Pa*m^3
    # = Pa/s
    du.pressure[cell_id] += du.mass[cell_id] / (cell_volumes[cell_id] / (R_gas * u.temp[cell_id]))
end
